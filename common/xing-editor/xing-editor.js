...
Component({
  ...
  methods: {
                ...
                wx.uploadFile({
                  ...
                  success: res => {
                    ...
                    if (data.flag) {
                      ...
                      nodeList.splice(index + 1, 0, node);
                      // 每个属性图片
                      textBufferPool.splice(index + 1, 0, data.src);
                      that.setData({
                        nodeList,
                        textBufferPool,
                      })
    ...
    deleteNode: function(e) {
      ...
      var textBufferPool = that.data.textBufferPool;
      var deleteNode = nodeList[index];
      if (deleteNode.name == "img"){
        that.deleteImage(nodeList, textBufferPool, index, deleteNode.attrs.id)
      }else{
        that.removeNode(nodeList, textBufferPool, index)
      }
    },
    /**
    * 删除图片
    */
    deleteImage: function (nodeList, textBufferPool,index,pid) {
      var that = this;
        wx.showModal({
          title: '提示',
          content: '确定要删除此图片吗？',
   ...
   * 移除节点
   */
    removeNode(nodeList, textBufferPool,index){
